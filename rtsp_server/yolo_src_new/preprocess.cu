...
// BGR
__global__ void warpaffine_kernel_gpu(uint8_t* src, size_t src_line_size,
...
__global__ void warpaffine_kernel_gpu_rgba(
    const uint8_t* src,
    ...
    int edge)
{
    ...
    float c0, c1, c2;   // src RGBA 裡的 R,G,B（先照 memory 順序讀）
    // 最後仍會轉成 RGB planar 輸出
    ...
    if (src_x <= -1 || src_x >= src_width || src_y <= -1 || src_y >= src_height) {
        ...
        uint8_t const_value[] = {const_value_st, const_value_st, const_value_st, 255};
        ...
        // 來源是 RGBA
        c0 = w1 * v1[0] + w2 * v2[0] + w3 * v3[0] + w4 * v4[0]; // R
        c1 = w1 * v1[1] + w2 * v2[1] + w3 * v3[1] + w4 * v4[1]; // G
        c2 = w1 * v1[2] + w2 * v2[2] + w3 * v3[2] + w4 * v4[2]; // B
    }
...
#if 0
// BGR
void cuda_preprocess_gpu(uint8_t* src_device, size_t src_step,
                         int src_width, int src_height,
                         ...
                         cudaStream_t stream)
{
    ...
    warpaffine_kernel_gpu<<<blocks, threads, 0, stream>>>(
        src_device,
        src_step,
        src_width,
        src_height,
                         ...
                         cudaStream_t stream)
{
    ...
    warpaffine_kernel_gpu_rgba<<<blocks, threads, 0, stream>>>(
        src_device,
        src_step,
        src_width,
        src_height,
