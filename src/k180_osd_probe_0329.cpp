			...
			if (count > 0) {
				...
				for (NvDsMetaList* l_frame = batch_meta->frame_meta_list;
					...
					for (int i = 0; i < count; ++i) {
						osd_add_detection_meta(batch_meta, frame_meta, exact_slot->det_host[i], BoxColor::White);
					}
				}
// tracking result，黃框
std::vector<Detection> track_boxes;
{
	std::lock_guard<std::mutex> lock(k180::runtime::tracking::g_tracks_mutex);
	track_boxes.reserve(k180::runtime::tracking::g_tracks_by_id.size());
	for (const auto& kv : k180::runtime::tracking::g_tracks_by_id) {
		track_boxes.push_back(kv.second.det);
	}
}
...
for (NvDsMetaList* l_frame = batch_meta->frame_meta_list;
    ...
    g_probe_rate.fb_det_sum.fetch_add(
        (uint64_t)fallback_local.size(), std::memory_order_relaxed);
