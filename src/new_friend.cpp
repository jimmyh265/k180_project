// https://docs.opencv.org/4.6.0/d9/dd8/samples_2cpp_2stitching_detailed_8cpp-example.html
// https://cloud.tencent.com/developer/article/1735256
// try_cuda failed: https://blog.csdn.net/stacowt/article/details/122908182
// try_cuda failed: https://github.com/opencv/opencv/issues/7344
// ./build/stitching_detailed ./pics/boat[1-5].jpg --try_cuda yes
#include <iostream>
#include <fstream>
#include <string>
...
#undef HAVE_OPENCV_XFEATURES2D  // fourd: xfeatures2d exception
#undef HAVE_OPENCV_CUDALEGACY   // fourd: for --try_cuda yes
// #define HAVE_OPENCV_CUDALEGACY   // fourd: for --try_cuda yes
...
// float match_conf = 0.05f;
#else
string features_type = "sift";
float match_conf = 0.3f;
// float match_conf = 0.03f;
#endif
...
Mat new_ssss(Mat *img_array){
	Mat cut_img[2];
	// Mat cut_img[0] = cv::Mat(1080, 1920, CV_8UC3, Scalar::all(0));
	// Mat cut_img[1] = cv::Mat(1080, 1920, CV_8UC3, Scalar::all(0));
	Mat result = cv::Mat(1080, 3840, CV_8UC3 );
	// img_array[0].copyTo(result(cv::Rect(0, 0, 1920, 1080)));
	// img_array[1].copyTo(result(cv::Rect(1920, 0, 1920, 1080)));
// #if 0
	int num_images = 2;
	/* ----- scale & features ----- */
    ...
    for (int i = 0; i < num_images; ++i)
    {
		...
		if( i==0 ){
			img(cv::Rect(img.cols/2, 0, img.cols/2, img.rows)).copyTo(cut_img[0](cv::Rect(img.cols/2, 0, img.cols/2, img.rows)));
		}
		if( i==1 ){
			// cut_img[1] = img(cv::Rect(0, 0, img.cols/2, img.rows));
			img(cv::Rect(0, 0, img.cols/2, img.rows)).copyTo(cut_img[1](cv::Rect(0, 0, img.cols/2, img.rows)));
		}
        // computeImageFeatures(finder, img, features[i]);
        computeImageFeatures(finder, cut_img[i], features[i]);
